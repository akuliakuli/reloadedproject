...
export default function BalanceWithdrawRequest({item}: any) {
    const router = useRouter();
    const [clicked, setClicked] = useState(false);
    const declineRequest = (id: any) => {
        axios.post(`/api/balance/withdraw/${id}`, {status: FillStatus.Declined}).then(() => {
          toast.success(':)')
          router.refresh();
        }).catch((error) => {
          toast.error(":(")
      ...
      const acceptRequest = (id: any) => {
        axios.post(`/api/balance/withdraw/${id}`, {status: FillStatus.Accepted}).then(() => {
          toast.success(':)')
          router.refresh();
        }).catch((error) => {
          toast.error(":(")
